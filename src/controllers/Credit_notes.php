...
class Credit_notes extends REST_Controller {
    ...
    function __construct() {
        ...
        $this->credit_notes_model = new Credit_notes_model();
    ...
    public function data_get($id = '') {
        ...
        $data = $this->credit_notes_model->get($id, [], $this->playground());
        if (!empty($data) && !empty($id)) {
            $data->items = $this->custom_fields_model->get_custom_data($data->items, "items", '', true, $this->playground());
        }
        // Check if the data store contains
        if ($data) {
            $data = $this->custom_fields_model->get_custom_data($data, "credit_note", $id, false, $this->playground());
    ...
    public function data_search_get($key = '') {
        $data = $this->credit_notes_model->get($key, [], $this->playground());
        if (!empty($data) && !empty($id)) {
            $data->items = $this->custom_fields_model->get_custom_data($data->items, "items", '', true, $this->playground());
        }
        $data = $this->custom_fields_model->get_relation_data_api('creditnotes', $key, $this->playground());
        // Check if the data store contains
        if ($data) {
            $data = $this->custom_fields_model->get_custom_data($data, "credit_note", "", false, $this->playground());
            // Set the response and exit
            $this->response($data, REST_Controller::HTTP_OK); // OK (200) being the HTTP response code            
        } else {
            // Set the response and exit
    ...
    public function data_delete($id = '') {
        ...
        if (empty($id) && !is_numeric($id)) {
            ...
            $is_exist = $this->credit_notes_model->get($id, $this->playground());
            if (is_object($is_exist)) {
                $output = $this->credit_notes_model->delete($id, $this->playground());
                if ($output === TRUE) {
                    // success
    ...
    public function data_post() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $id = $this->credit_notes_model->add($data, $this->playground());
            if ($id > 0 && !empty($id)) {
                // success
    ...
    public function validate_creditnotes_number($number, $credit_notes_id) {
        ...
        if (!empty($credit_notes_id)) {
            $data = $this->credit_notes_model->get($credit_notes_id, [], $this->playground());
            if (!empty($data) && !empty($id)) {
                $data->items = $this->custom_fields_model->get_custom_data($data->items, "items", '', true, $this->playground());
            }
        ...
        if (total_rows(db_prefix() . ($this->playground() ? 'playground_' : '') . 'creditnotes', ['YEAR(date)' => date('Y', strtotime(to_sql_date($date))), 'number' => $number, ]) > 0) {
            return FALSE;
        } else {
            return TRUE;
        }
        ...
        if (empty($id) && !is_numeric($id)) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (is_object($is_exist)) {
                    $data = $this->input->post();
                    $data['isedit'] = "";
                    $success = $this->credit_notes_model->update($data, $id, $this->playground());
                    if ($success == true) {
