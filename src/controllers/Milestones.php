...
class Milestones extends REST_Controller {
    ...
    public function data_get($id = '') {
        ...
        $data = $this->misc_model->get_milestones($id, [], $this->playground());
    ...
    public function data_search_get($key = '') {
        // If the id parameter doesn't exist return all the
        $data = $this->custom_fields_model->get_relation_data_api('milestones', $key, $this->playground());
        // Check if the data store contains
        if ($data) {
            // Set the response and exit
            $this->response($data, REST_Controller::HTTP_OK); // OK (200) being the HTTP response code
    ...
    public function data_post() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $insert_data = ['name' => $this->input->post('name', TRUE), 'due_date' => $this->input->post('due_date', TRUE), 'project_id' => $this->input->post('project_id', TRUE), 'description' => $this->misc_model->value($this->input->post('description', TRUE)), 'description_visible_to_customer' => $this->misc_model->value($this->input->post('description_visible_to_customer', TRUE)), 'milestone_order' => $this->misc_model->value($this->input->post('milestone_order', TRUE)) ];
            // insert data
            $projects_model = new Projects_model();
            $output = $projects_model->add_milestone($insert_data, $this->playground());
            if ($output > 0 && !empty($output)) {
                // success
                $message = array('status' => TRUE, 'message' => 'Milestone add successful.');
    ...
    public function data_delete($id = '') {
        ...
        if (empty($id) && !is_numeric($id)) {
            ...
            // delete data
            $projects_model = new Projects_model();
            $output = $projects_model->delete_milestone($id, $this->playground());
            if ($output === TRUE) {
                // success
    ...
    public function data_put($id = '') {
        ...
        if (empty($id) && !is_numeric($id)) {
            ...
            // update data
            $projects_model = new Projects_model();
            $output = $projects_model->update_milestone($update_data, $id, $this->playground());
            if ($output > 0 && !empty($output)) {
                // success
