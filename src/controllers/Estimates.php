...
class Estimates extends REST_Controller {
    ...
    public function data_get($id = '') {
        ...
        if ($data) {
            $data = $this->custom_fields_model->get_custom_data($data, "estimate", $id, false, $this->playground());
    ...
    public function data_search_get($key = '') {
        $data = $this->custom_fields_model->get_relation_data_api('estimates', $key, $this->playground());
        // Check if the data store contains
        if ($data) {
            $data = $this->custom_fields_model->get_custom_data($data, "estimate", "", false, $this->playground());
            // Set the response and exit
            $this->response($data, REST_Controller::HTTP_OK); // OK (200) being the HTTP response code            
        } else {
            // Set the response and exit
    ...
    public function data_delete($id = '') {
        ...
        if (empty($id) && !is_numeric($id)) {
            ...
            $is_exist = $this->estimates_model->get($id, $this->playground());
            if (is_object($is_exist)) {
                $output = $this->estimates_model->delete($id, $this->playground());
                if ($output === TRUE) {
                    // success
    ...
    public function data_post() {        
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $id = $this->estimates_model->add($data, $this->playground());
            if ($id > 0 && !empty($id)) {
                // success
    ...
    public function validate_estimate_number($number, $estimateid) {
        ...
        if (!empty($estimateid)) {
            $data = $this->estimates_model->get($estimateid, $this->playground());
        ...
        if (total_rows(db_prefix() . ($this->playground() ? 'playground_' : '') . 'estimates', ['YEAR(date)' => date('Y', strtotime(to_sql_date($date))), 'number' => $number, ]) > 0) {
            return FALSE;
        } else {
            return TRUE;
        }
        ...
        if (empty($id) && !is_numeric($id)) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (is_object($is_exist)) {
                    $data = $this->input->post();
                    $data['isedit'] = "";
                    $success = $this->estimates_model->update($data, $id, $this->playground());
                    if ($success == true) {
