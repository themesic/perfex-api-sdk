...
class Proposals extends REST_Controller {
    ...
    public function data_get($id = '') {
        ...
        $data = $this->proposals_model->get($id, [], false, $this->playground());
        if (!empty($data) && !empty($id)) {
            $data->items = $this->custom_fields_model->get_custom_data($data->items, 'items', '', true, $this->playground());
        }
        // Check if the data store contains
        if ($data) {
            $data = $this->custom_fields_model->get_custom_data($data, "proposal", $id, false, $this->playground());
    ...
    public function data_search_get($key = '') {
        $data = $this->custom_fields_model->get_relation_data_api('proposals', $key, $this->playground());
        // Check if the data store contains
        if ($data) {
            $data = $this->custom_fields_model->get_custom_data($data, "proposal", "", false, $this->playground());
            // Set the response and exit
            $this->response($data, REST_Controller::HTTP_OK); // OK (200) being the HTTP response code            
        } else {
            // Set the response and exit
    ...
    public function data_delete($id = '') {
        ...
        if (empty($id) && !is_numeric($id)) {
            ...
            $is_exist = $this->proposals_model->get($id, [], false, $this->playground());
            if (is_object($is_exist)) {
                $output = $this->proposals_model->delete($id, $this->playground());
                if ($output === TRUE) {
                    // success
    ...
    public function data_post() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $id = $this->proposals_model->add($data, $this->playground());
            if ($id > 0 && !empty($id)) {
                // success
        ...
        if (empty($id) && !is_numeric($id)) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (is_object($is_exist)) {
                    $data = $this->input->post();
                    $data['isedit'] = "";
                    $success = $this->proposals_model->update($data, $id, $this->playground());
                    if ($success == true) {
