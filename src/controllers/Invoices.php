...
class Invoices extends REST_Controller {
	...
	public function data_get($id = '') {
        ...
        $data = $this->invoices_model->get($id, $this->playground());
		...
		if ($data) {
			...
			$data = $this->custom_fields_model->get_custom_data($data, "invoice", $id, false, $this->playground());
    ...
    public function data_search_get($key = '') {
        $data = $this->custom_fields_model->get_relation_data_api('invoices', $key, $this->playground());
        // Check if the data store contains
        if ($data) {
            $data = $this->custom_fields_model->get_custom_data($data, "invoice", "", false, $this->playground());
            // Set the response and exit
            $this->response($data, REST_Controller::HTTP_OK); // OK (200) being the HTTP response code            
        } else {
            // Set the response and exit
    ...
    public function data_post() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $id = $this->invoices_model->add($data, $this->playground());
            if ($id > 0 && !empty($id)) {				
                // success
    ...
    public function data_delete($id = '') {
        ...
        if (empty($id) && !is_numeric($id)) {
            ...
            $is_exist = $this->invoices_model->get($id, $this->playground());
            if (is_object($is_exist)) {
                $output = $this->invoices_model->delete($id, $this->playground());
                if ($output === TRUE) {
                    // success
        ...
        if (empty($id) && !is_numeric($id)) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (is_object($is_exist)) {
                    $data = $this->input->post();
                    $data['isedit'] = "";
                    $success = $this->invoices_model->update($data, $id, $this->playground());
                    if ($success == true) {
    ...
    public function validate_invoice_number($number, $invoiceid) {
        ...
        if (total_rows(db_prefix() . ($this->playground() ? 'playground_' : '') . 'invoices', [
