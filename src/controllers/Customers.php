...
class Customers extends REST_Controller {
    ...
    public function data_get($id = '') {
        ...
        $data = $this->clients_model->get( $id, [], $this->playground());
        // Check if the data store contains
        if ($data) {
            $data = $this->custom_fields_model->get_custom_data($data, "customers", $id, false, $this->playground());
    ...
    public function data_search_get($key = '') {
        // If the id parameter doesn't exist return all the
        $data = $this->custom_fields_model->get_relation_data_api('customer', $key, $this->playground());
        // Check if the data store contains
        if ($data) {
            $data = $this->custom_fields_model->get_custom_data($data, "customers", "", false, $this->playground());
            // Set the response and exit
            $this->response($data, REST_Controller::HTTP_OK); // OK (200) being the HTTP response code            
        } else {
            // Set the response and exit
    ...
    public function data_post() {        
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $groups_in = $this->misc_model->value($this->input->post('groups_in', TRUE));
            $insert_data = ['company' => $this->input->post('company', TRUE), 'vat' => $this->misc_model->value($this->input->post('vat', TRUE)), 'phonenumber' => $this->misc_model->value($this->input->post('phonenumber', TRUE)), 'website' => $this->misc_model->value($this->input->post('website', TRUE)), 'default_currency' => $this->misc_model->value($this->input->post('default_currency', TRUE)), 'default_language' => $this->misc_model->value($this->input->post('default_language', TRUE)), 'address' => $this->misc_model->value($this->input->post('address', TRUE)), 'city' => $this->misc_model->value($this->input->post('city', TRUE)), 'state' => $this->misc_model->value($this->input->post('state', TRUE)), 'zip' => $this->misc_model->value($this->input->post('zip', TRUE)), 'country' => $this->misc_model->value($this->input->post('country', TRUE)), 'billing_street' => $this->misc_model->value($this->input->post('billing_street', TRUE)), 'billing_city' => $this->misc_model->value($this->input->post('billing_city', TRUE)), 'billing_state' => $this->misc_model->value($this->input->post('billing_state', TRUE)), 'billing_zip' => $this->misc_model->value($this->input->post('billing_zip', TRUE)), 'billing_country' => $this->misc_model->value($this->input->post('billing_country', TRUE)), 'shipping_street' => $this->misc_model->value($this->input->post('shipping_street', TRUE)), 'shipping_city' => $this->misc_model->value($this->input->post('shipping_city', TRUE)), 'shipping_state' => $this->misc_model->value($this->input->post('shipping_state', TRUE)), 'shipping_zip' => $this->misc_model->value($this->input->post('shipping_zip', TRUE)), 'shipping_country' => $this->misc_model->value($this->input->post('shipping_country', TRUE)) ];
            ...
            // insert data
            $output = $this->clients_model->add($insert_data, false, $this->playground());
    ...
    public function data_delete($id = '') {
        ...
        if (empty($id) && !is_numeric($id)) {
            ...
            // delete data
            $output = $this->clients_model->delete($id, $this->playground());
            if ($output === TRUE) {
                // success
    ...
    public function data_put($id = '') {
        ...
        if (empty($id) && !is_numeric($id)) {
            ...
            // update data
            $output = $this->clients_model->update($update_data, $id, false, $this->playground());
            if ($output > 0 && !empty($output)) {
                // success
