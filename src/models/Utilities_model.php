...
class Utilities_model extends \App_Model {
    ...
    public function get_calendar_data($start, $end, $client_id = '', $contact_id = '', $filters = false, $playground = false) {
        ...
        if (get_option('show_invoices_on_calendar') == 1 && !$ff || $ff && array_key_exists('invoices', $filters)) {
            ...
            $this->db->select('duedate as date,number,id,clientid,hash,' . $clients_model->get_sql_select_client_company('company', $playground));
        ...
        if (get_option('show_estimates_on_calendar') == 1 && !$ff || $ff && array_key_exists('estimates', $filters)) {
            ...
            $clients_model = new Clients_model();
            $this->db->select('number,id,clientid,hash,CASE WHEN expirydate IS NULL THEN date ELSE expirydate END as date,' . $clients_model->get_sql_select_client_company('company', $playground), false);
        ...
        if (get_option('show_contracts_on_calendar') == 1 && !$ff || $ff && array_key_exists('contracts', $filters)) {
            $clients_model = new Clients_model();
            $this->db->select('hash, subject as title, dateend, datestart, id, client, content, ' . $clients_model->get_sql_select_client_company('company', $playground));
        ...
        if (get_option('show_projects_on_calendar') == 1 && !$ff || $ff && array_key_exists('projects', $filters)) {
            $clients_model = new Clients_model();
            $this->db->select('name as title,id,clientid, CASE WHEN deadline IS NULL THEN start_date ELSE deadline END as date,' . $clients_model->get_sql_select_client_company('company', $playground), false);
