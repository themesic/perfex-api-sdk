...
use PerfexApiSdk\Models\Client_vault_entries_model;
use PerfexApiSdk\Models\Client_groups_model;
use PerfexApiSdk\Models\Statement_model;
use PerfexApiSdk\Models\Roles_model;

use PerfexApiSdk\Models\Authentication_model;
...
class Clients_model extends \App_Model {
    ...
    protected $client_vault_entries_model;
    protected $client_groups_model;
    ...
    public function __construct() {
        ...
        $this->client_vault_entries_model = new Client_vault_entries_model();
        $this->client_groups_model = new Client_groups_model();
        $this->statement_model = new Statement_model();
    ...
    */
    ...
    public function add($data, $withContact = false, $playground = false) {
        ...
        if ($client_id) {
            if (count($custom_fields) > 0) {
                ...
                $custom_fields_model->handle_custom_fields_post($client_id, $custom_fields, false, $playground);
            }
            /**
    ...
    public function add_contact($data, $customer_id, $not_manual_request = false, $playground = false) {
        ...
        if ($contact_id) {
            if (isset($custom_fields)) {
                $custom_fields_model = new Custom_fields_model();
                $custom_fields_model->handle_custom_fields_post($contact_id, $custom_fields, false, $playground);
            }
            ...
            if (defined('CONTACT_REGISTERING')) {
                ...
                $tickets_model = new Tickets_model();
                $tickets_model->transfer_email_tickets_to_contact($data['email'], $contact_id, $playground);
            }
    ...
    public function update_company_details($data, $id, $playground = false) {
        ...
        if (isset($data['custom_fields'])) {
            $custom_fields = $data['custom_fields'];
            $custom_fields_model = new Custom_fields_model();
            if ($custom_fields_model->handle_custom_fields_post($id, $custom_field, false, $playground)) {
                $affectedRows++;
            }
            unset($data['custom_fields']);
        }
    ...
    public function delete($id, $playground = false) {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            foreach ($tickets as $ticket) {
                $tickets_model->delete($ticket['ticketid'], $playground);
            }
            ...
            if (is_gdpr() && get_option('gdpr_on_forgotten_remove_invoices_credit_notes') == '1') {
                ...
                foreach ($invoices as $invoice) {
                    $invoices_model->delete($invoice['id'], true);
                }
                ...
                foreach ($credit_notes as $credit_note) {
                    $credit_notes_model->delete($credit_note['id'], true);
                }
            } else if (is_gdpr()) {
                $this->db->where('clientid', $id);
            ...
            if (is_gdpr() && get_option('gdpr_on_forgotten_remove_estimates') == '1') {
                ...
                foreach ($estimates as $estimate) {
                    $estimates_model->delete($estimate['id'], true);
                }
            } else if (is_gdpr()) {
                $this->db->where('clientid', $id);
            ...
            $subscriptions_model = new Subscriptions_model();
            $this->db->where('clientid', $id);
            $subscriptions = $this->db->get(db_prefix() . ($playground ? 'playground_' : '') . 'subscriptions')->result_array();
            foreach ($subscriptions as $subscription) {
                $subscriptions_model->delete($subscription['id'], true, $playground);
            }
            ...
            foreach ($contracts as $contract) {
                $contracts_model->delete($contract['id']);
            }
            ...
            foreach ($expenses as $expense) {
                $expenses_model->delete($expense['id'], true);
            }
            ...
            foreach ($projects as $project) {
                $projects_model->delete($project['id'], $playground);
            }
        }
        if ($affectedRows > 0) {
    ...
    public function delete_contact($id, $playground = false) {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            if (is_dir($misc_model->get_upload_path_by_type('contact_profile_images', $playground) . $id)) {
                delete_dir($misc_model->get_upload_path_by_type('contact_profile_images', $playground) . $id);
            }
            ...
            foreach ($files as $file) {
                $projects_model->remove_file($file['id'], false, $playground);
            }
            $this->db->where('contact_id', $id);
            $discussions = $this->db->get(db_prefix() . ($playground ? 'playground_' : '') . 'projectdiscussions')->result_array();
            foreach ($discussions as $discussion) {
                $projects_model->delete_discussion($discussion['id'], false, $playground);
            }
            $this->db->where('contact_id', $id);
            ...
            foreach ($discussionsComments as $comment) {
                $projects_model->delete_discussion_comment($comment['id'], false, $playground);
            }
            $this->db->where('contact_id', $id);
            $this->db->delete(db_prefix() . ($playground ? 'playground_' : '') . 'user_meta');
    ...
    public function delete_attachment($id, $playground = false) {
        ...
        if ($attachment) {
            if (empty($attachment->external)) {
                $relPath = $misc_model->get_upload_path_by_type('customer', $playground) . $attachment->rel_id . '/';
            ...
            if (is_dir($misc_model->get_upload_path_by_type('customer', $playground) . $attachment->rel_id)) {
                // Check if no attachments left, so we can delete the folder also
                $other_attachments = list_files($misc_model->get_upload_path_by_type('customer', $playground) . $attachment->rel_id);
                if (count($other_attachments) == 0) {
                    delete_dir($misc_model->get_upload_path_by_type('customer', $playground) . $attachment->rel_id);
                }
            }
        }
        return $deleted;
    ...
    public function mark_email_as_verified($id, $playground = false) {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $tickets_model = new Tickets_model();
            $tickets_model->transfer_email_tickets_to_contact($contact->email, $contact->id, $playground);
            return true;
        }
        return false;
    }
    ...
    public function delete_contact_profile_image($id, $playground = false) {
        ...
        if (file_exists($misc_model->get_upload_path_by_type('contact_profile_images', $playground) . $id)) {
            delete_dir($misc_model->get_upload_path_by_type('contact_profile_images', $playground) . $id);
        }
        $this->db->where('id', $id);
    ...
    * @return string
    */
    public function get_sql_select_client_company($as = 'company', $playground)
    {
